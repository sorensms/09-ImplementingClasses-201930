...
class Point(object):
    ...
    def move_by(self,dx,dy):
        ...
        self.y=self.y+dy
        self.count=self.count+1
        self.countdx=dx+self.countdx
        self.countdy=dy+self.countdy
        self.distance=math.sqrt((self.countdx)**2+(self.countdy)**2)+self.distance
    ...
    def closer_to(self,p2,p3):
        if math.sqrt((self.x-p2.x)**2+(self.y-p2.y)**2)> ((self.x-p3.x)**2+(self.y-p3.y)**2):
            return "Point ({},{})".format(p2.x,p2.y)
        else:
            return "Point ({},{})".format(p3.x,p3.y)
